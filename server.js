...
async function seedInitialPubs() {
  const pubs = [
    {
      title: 'Primeira Publicação',
      content: 'Bem-vindo ao Seeeshow!',
      redirectUrl: 'https://google.com'
    },
    {
      title: 'Dica de Hoje',
      content: 'Clique no próximo para ganhar ponto.',
      redirectUrl: 'https://youtube.com'
    },
    {
      title: 'Pub de Exemplo',
      content: 'Isso é só uma demonstração.',
      redirectUrl: 'https://github.com'
    },
    {
      title: 'Ganhe Visualizações',
      content: 'Veja e publique também!',
      redirectUrl: 'https://vercel.com'
    },
    {
      title: 'Última Pub de Teste',
      content: 'Vamos ver como isso funciona.',
  ...
  for (const pub of pubs) {
    try {
      const id = generateId();
      const content = Buffer.from(JSON.stringify(pub)).toString('base64');
      await axios.put(`${GITHUB_API}/${id}.json`, {
        message: `Seed: ${pub.title}`,
        content: content,
        branch: GITHUB_BRANCH
      }, {
        headers: {
      ...
      console.log(`✅ Criado: ${pub.title}`);
    } catch (err) {
      console.error(`❌ Erro ao criar ${pub.title}:`, err.message);
    }
  }
}
...
app.listen(PORT, () => {
  ...
  seedInitialPubs(); // ⚠️ Só rodar uma vez
});
